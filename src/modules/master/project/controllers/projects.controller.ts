...
export class MasterProjectsController {
  ...
  findAll(@Query() query: PaginationDto): Promise<PaginatedResponseDto<MasterProjectResponseDto>> {
  ...
  findOne(@Param('id') id: string): Promise<BaseResponseDto<MasterProjectResponseDto>> {
  ...
  async remove(@Param('id', ParseIntPipe) id: number): Promise<BaseResponseDto<SuccessResponseDto>> {
