...
export class ProjectActivitiesService {
  ...
  async update(id: number, dto: UpdateProjectActivityDto): Promise<BaseResponseDto<ProjectActivityResponseDto>> {
    const activity = await this.activityRepo.findOne({
      where: { id },
      relations: { assignedTo: true },
    });
  ...
  async updateProgress(id: number, dto: UpdateProgressDto): Promise<BaseResponseDto<ProjectActivityResponseDto>> {
    const activity = await this.activityRepo.findOne({
      where: { id },
      relations: { assignedTo: true },
    });
  ...
  async remove(id: number): Promise<BaseResponseDto<null>> {
    ...
    return { success: true, data: null, message: 'Activity deleted successfully' };
  }
}
