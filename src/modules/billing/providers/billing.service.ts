...
export class BillingService {
  ...
  async generatePreview(
    ...
    const hasProjectIds =
      isProjectType && dto.projectIds && dto.projectIds.length > 0;
    const hasSupportTicketIds =
      isSupportType && dto.supportTicketIds && dto.supportTicketIds.length > 0;

    if (!hasProjectIds && !hasSupportTicketIds) {
      throw new NotFoundException(
        `No valid items found for ${dto.billingType} billing type`,
      );
    }
    ...
    if (hasProjectIds && projects.length > 0) {
      ...
      for (const member of members) {
        ...
        const memberActivities = filteredActivities.filter(
          (act) =>
            Number(act.assignedToId) === Number(member.userId) &&
            Number(act.projectId) === Number(member.projectId),
        );
        ...
        if (
          ...
          !groups[key].memberNames.includes(member.user.fullName)
        ) {
      ...
      for (const key of Object.keys(groups)) {
        ...
        const rate = roleRate
          ? Number(
              isSupport
                ? roleRate.ratePerMandaySupport
                : roleRate.ratePerMandayProject,
            )
          : 0;
    ...
    if (hasSupportTicketIds) {
      ...
      for (const ticket of supportTickets) {
        for (const assignee of ticket.assignees || []) {
          ...
          detailItems.push({
            projectId: ticket.masterProjectId
              ? Number(ticket.masterProjectId)
              : null,
    ...
    const totalMandays = detailItems.reduce(
      (sum, item) => sum + item.mandays,
      0,
    );
  ...
  async createBilling(
    ...
    const lastBill = await this.billingRepo
  ...
  async remove(id: number): Promise<BaseResponseDto<void>> {
    ...
    await this.billingRepo.manager.transaction(async (manager) => {
      ...
      await manager.query(`DELETE FROM billing_projects WHERE billing_id = ?`, [
        id,
      ]);
